-----------------------------------------------------------------------------
-- |
-- Module      :  Language.Haskell.Exts.Annotated.PrettyWithComments
...
newtype (Monad m) => DocMT s m a = DocMT { runDocMT :: s -> m a }
...
instance Pretty GuardedRhs where
        pretty (GuardedRhs _pos guards ppBody) =
                myFsep $ [char '|'] ++
                    (punctuate comma . map pretty $ guards) ++ [equals, pretty ppBody]
...
instance Pretty Exp where
        ...
        -- lambda stuff
        prettyPrec p (InfixApp a op b) = parensIf (p > 2) $ myFsep [prettyPrec 2 a,
            pretty op, prettyPrec 1 b]
        ...
        -- Arrows
        prettyPrec p (Proc _ pat e) = parensIf (p > 1) $ myFsep $ [text "proc",
            pretty pat, text "->", pretty e]
        prettyPrec p (LeftArrApp l r)      = parensIf (p > 0) $ myFsep $ [pretty l,
            text "-<",  pretty r]
        prettyPrec p (RightArrApp l r)     = parensIf (p > 0) $ myFsep $ [pretty l,
            text ">-",  pretty r]
        prettyPrec p (LeftArrHighApp l r)  = parensIf (p > 0) $ myFsep $ [pretty l,
            text "-<<", pretty r]
        prettyPrec p (RightArrHighApp l r) = parensIf (p > 0) $ myFsep $ [pretty l,
            text ">>-", pretty r]
...
instance Pretty GuardedAlt where
        pretty (GuardedAlt _pos guards body) =
                myFsep $ char '|': (punctuate comma . map pretty $ guards) ++
                    [text "->", pretty body]
        ...
        pretty (A.XmlHybrid pos mbHead os imp decls n attrs mattr cs) =
                ...
                myVcat $ map pretty os ++ [text "<%"] ++
                    ...
                    (map pretty imp ++ map pretty decls ++
                        [let ax = maybe [] (return . pretty) mattr
                          in hcat $
                             (myFsep $ (char '<' <> pretty n):
                                map pretty attrs ++ ax ++ [char '>']):
                                    map pretty cs ++ [myFsep $ [text "</" <> pretty n, char '>']]])
